...
<style>
    ...
    .city-profit { font-weight: bold; font-size: 0.9rem; }
    .stButton>button {
        width: 100%;
...
if btn:
    ...
    try:
        ...
        for entry in resp_venda:
            ...
            if city == "Black Market": p_final = entry["sell_price_min"]
                ...
                # Lógica para cidades reais: prioriza sell_price_min se for razoável, senão buy_price_max
        ...
        for recurso, qtd in [(d[1], d[2]), (d[3], d[4])]:
            ...
            for rid in ids_recurso_variantes(tier, recurso, encanto):
                if rid in precos_recursos:
                    ...
                    dets.append(f"{qtd*quantidade}x {nome_rec}: {int(info['price']):,} ({info['city']})")
                    found = True; break
            if not found: valid = False
        
        if not valid: continue
        
        # Custo de Artefato (Busca o menor preço nas cidades reais)
        if d[5]:
    ...
    # 5. Exibição
    if not resultados:
        st.warning("⚠️ Nenhum item lucrativo encontrado nas cidades analisadas.")
    else:
        ...
        for res in resultados:
            ...
            <div class="item-card-custom" style="border-left: 8px solid {cor};">
                ...
                <div style="margin-bottom: 15px;">
                    <b>Investimento Total:</b> {res['custo']:,} Silver | 📍 <b>Foco:</b> {identificar_cidade_bonus(res['nome'])}
                </div>
                ...
                <div style="background: rgba(0,0,0,0.3); padding: 12px; border-radius: 8px; font-size: 0.85rem; border: 1px solid rgba(255,255,255,0.1);">
                    📦 <b>Detalhamento:</b> {" | ".join(res['detalhes'])}
                </div>
            </div>
            """, unsafe_allow_html=True)
