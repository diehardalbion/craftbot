...
ITENS_DB = {
    ...
    "ESCUDO DE SARCOFAGO": ["OFF_SHIELD_UNDEAD", "Tábuas de Pinho", 4, "Barra de Aço", 4, "ARTEFACT_OFF_SHIELD_UNDEAD", 1],
    "ESCUDO DA FACE DEMONIACA": ["OFF_SHIELD_HELL", "Tábuas de Pinho", 4, "Barra de Aço", 4, "ARTEFACT_OFF_SHIELD_HELL", 1],
    "QUEBRA-FEITIÇOS": ["OFF_SPIKEDSHIELD_MORGANA", "Tábuas de Pinho", 4, "Barra de Aço", 4, "ARTEFACT_OFF_SPIKEDSHIELD_MORGANA", 1],
    "ESCUDO ASTAL": ["OFF_SHIELD_AVALON", "Tábuas de Pinho", 4, "Barra de Aço", 4, "ARTEFACT_OFF_SHIELD_AVALON", 1],
    "ESCUDO DE ÉGIDE FORJADO": ["OFF_SHIELD_CRYSTAL", "Tábuas de Pinho", 4, "Barra de Aço", 4, "QUESTITEM_TOKEN_CRYSTAL_SHIELD", 1],
...
if st.session_state.get("buscar"):
    ...
    if not itens_filtrados:
        ...
        try:
            res = requests.get(f"{API_URL}{','.join(list(set(ids_para_api)))}?locations={','.join(CIDADES)}")
            data = res.json()
            precos = {}
            for e in data:
                iid = e['item_id']
                if iid not in precos: precos[iid] = {}
                precos[iid][e['city']] = {'p': e['sell_price_min'], 't': e['sell_price_min_date']}

            st.subheader(f"📊 Análise {categoria} - T{tier}.{encanto}")
            
            for nome, d in itens_filtrados.items():
                suf = f"@{encanto}" if encanto > 0 else ""
                id_i, id_m1 = f"T{tier}_{d[0]}{suf}", f"T{tier}_{RECURSO_MAP[d[1]]}{suf}"
                n_m1 = NOMES_RECURSOS_TIER[d[1]][tier]
                
                p_m1 = precos.get(id_m1, {}).get("Thetford", {}).get('p', 0)
                custo = p_m1 * d[2]
                ...
                if d[3]:
                    ...
                    p_m2 = precos.get(id_m2, {}).get("Thetford", {}).get('p', 0)
                    custo += (p_m2 * d[4])
                    detalhe_mat += f" | {d[4]}x T{tier}.{encanto} {n_m2}: {p_m2:,.0f}"
                
                if d[5]:
                    p_art = precos.get(f"T{tier}_{d[5]}", {}).get("Caerleon", {}).get('p', 0)
                    custo += p_art
                    detalhe_mat += f" | Artefato: {p_art:,.0f}"

                custo = (custo * (1 + taxa_loja/100)) * quantidade
                venda_u = precos.get(id_i, {}).get("Black Market", {}).get('p', 0)
                h_venda = precos.get(id_i, {}).get("Black Market", {}).get('t', "N/A")
                ...
                <div class="item-card-custom" style="border-left: 8px solid {cor};">
                    ...
                    <div style="font-size: 1.05rem; margin-bottom: 8px;">
                        ...
                        <br><b>Investimento:</b> {custo:,.0f} | <b>Venda Líquida (BM):</b> {venda_l:,.0f}
                    </div>
